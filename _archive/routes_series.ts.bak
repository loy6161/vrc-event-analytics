...
// GET /api/series — シリーズマスタ一覧（色・市民権対象・イベント数・最終開催日つき）
router.get('/', async (_req: Request, res: Response) => {
  try {
    ok(res, await getSeriesList())
  } catch (err: any) { fail(res, err.message) }
})

// PATCH /api/series/:name — メタ更新（color / citizenship_target / sort_order）
router.patch('/:name', async (req: Request, res: Response) => {
  ...
  try {
    await updateSeriesMeta(name, patch)
    ok(res, { name, ...patch })
  } catch (err: any) { fail(res, err.message) }
})

// POST /api/series/:name/rename — 改名（全イベントへ自動反映）
...
// DELETE /api/series/:name — マスタ削除（該当イベントは未分類に戻す）
router.delete('/:name', async (req: Request, res: Response) => {
  const name = decodeURIComponent(req.params.name)
  try {
    await deleteSeriesMaster(name)
    ok(res, { name })
  } catch (err: any) { fail(res, err.message) }
})
